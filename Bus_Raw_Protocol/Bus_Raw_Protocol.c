...
int Generate_CheckSum(unsigned char *Pack, unsigned char Start_Index,
		unsigned char Lens) {
	unsigned char i;
...
int Array2Packet(int rs485_num, UART_Handle uart) {
	...
	while (rs485_ptr - old_ptr < rs485_num) {
		...
		{
			checksum = Generate_CheckSum((unsigned char*) receive_array, 3,
					receive_array[2]);
			...
			if (*(rs485_ptr) == checksum) {
				...
				case ctJOIN_Req:	//Join Request
				{
					...
					if (rs485_num == 14) {
						...
						UARTprintf(
								(const char*) "Join_Request(Assign BUS ID)Command Type:%d Response ERROR\n",rs485_num);
					}
					break;
				}
...
int Request_PV_Value(UART_Handle uart) {
	...
	for (Bus_ID_for_PV_Value = 0; Bus_ID_for_PV_Value < JB_Count;
			Bus_ID_for_PV_Value++) {
		if (member_table[Bus_ID_for_PV_Value].Valid == SJBStartCycle) {
			...
			Request_PV_Value_Buffer[6] = Generate_CheckSum(
					Request_PV_Value_Buffer, 3, Request_PV_Value_Buffer[2]);
			...
			if (num == 21) {
				...
				UARTprintf(
						(const char*) "Bus_ID %d num=%d Incomplete packets for Request_PV_Value \n",
						Bus_ID_for_PV_Value,num);
				member_table[Bus_ID_for_PV_Value].Valid =
						member_table[Bus_ID_for_PV_Value].Valid - 1;
			}
		} else {
...
int Response_PV_Value(uint8* Buffer_Array) {
	...
	for (i = 0; i < 4; i++)
		pv_value_table[Bus_ID_for_PV_Value].Power_Energy[i] = Buffer_Array[11
				+ i];
	for (i = 0; i < 4; i++)
		pv_value_table[Bus_ID_for_PV_Value].Alert_State[i] =
				Buffer_Array[15 + i];
	return 0;
}
...
int Request_PV_Info(UART_Handle uart) {
	...
	for (Bus_ID_for_PV_Info = 0; Bus_ID_for_PV_Info < JB_Count;
			Bus_ID_for_PV_Info++) {
		if (member_table[Bus_ID_for_PV_Value].Valid > 0) {
			Request_PV_Info_Buffer[4] = Bus_ID_for_PV_Info;
			Request_PV_Info_Buffer[6] = Generate_CheckSum(
					Request_PV_Info_Buffer, 3, Request_PV_Info_Buffer[2]);
			...
			do {
				...
				if (num == 118 ) {
					Array2Packet(num, uart);
					error++;
					if (pv_info_table[Bus_ID_for_PV_Info].Serial_Number[0]
							!= 0xFF)
						break;
					else
						UARTprintf(
								(const char*) ">>>>>Bus_ID %d Incomplete packets for Request_PV_Info <<<< \n",
								Bus_ID_for_PV_Info);
						;
				} else {
					error++;
					UARTprintf(
							(const char*) ">>>>>Bus_ID %d NUM = %d Incomplete packets for Request_PV_Info<<<<< \n",
							Bus_ID_for_PV_Info,num);
					/*for(i=0;i<6;i++)
...
void Response_PV_Info(uint8* Buffer_Array) {
	...
	for (i = 6; i < 30; i++)
		pv_info_table[Bus_ID_for_PV_Info].Serial_Number[i - 6] = Buffer_Array[6
				+ i];
	for (i = 30; i < 54; i++)
		pv_info_table[Bus_ID_for_PV_Info].Firmware_Version[i - 30] =
				Buffer_Array[6 + i];
	for (i = 54; i < 78; i++)
		pv_info_table[Bus_ID_for_PV_Info].Hardware_Version[i - 54] =
				Buffer_Array[6 + i];
	for (i = 78; i < 102; i++)
		pv_info_table[Bus_ID_for_PV_Info].Device_Specification[i - 78] =
				Buffer_Array[6 + i];
	for (i = 102; i < 110; i++)
		pv_info_table[Bus_ID_for_PV_Info].Manufacture_Date[i - 102] =
				Buffer_Array[6 + i];
}
...
int Check_Mac(uint8* Buffer_Array) {
	...
	//check Vailid
	if (member_table[Index].MAC[5] == Buffer_Array[11]
			&& member_table[Index].MAC[4] == Buffer_Array[10]
			&& member_table[Index].MAC[3] == Buffer_Array[9]
			&& member_table[Index].MAC[2] == Buffer_Array[8]
			&& member_table[Index].MAC[1] == Buffer_Array[7]
			&& member_table[Index].MAC[0] == Buffer_Array[6]) {
		Bus_ID = Index;
...
void Check_JB_Number() {
	...
	for (i = 0; i < Number_of_JB; i++) {
		if (member_table[i].Valid > 0)
			JB_Number++;
		if (member_table[i].MAC[0] != 0 || member_table[i].MAC[1] != 0
				|| member_table[i].MAC[2] != 0 || member_table[i].MAC[3] != 0
