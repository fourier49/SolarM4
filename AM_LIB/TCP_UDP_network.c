...
int TCP_transmission(char *pbuf, int size) {
	...
	setsockopt(server_sock, SOL_SOCKET, SO_SNDTIMEO, &server_timeout,
			sizeof(server_timeout));
	setsockopt(server_sock, SOL_SOCKET, SO_RCVTIMEO, &server_timeout,
			sizeof(server_timeout));
...
int AM_JoinRequest() {
	...
	sprintf(server_ip, "%d.%d.%d.%d", G_ENVCONFIG.IP[0], G_ENVCONFIG.IP[1],
			G_ENVCONFIG.IP[2], G_ENVCONFIG.IP[3]);
	server_IPAddr = inet_addr(server_ip);
...
//	System_flush();
	...
	recv_num = recv(server_sock, Recv_Array, AMJoinResponsePKLength,
			MSG_WAITALL);
	if (recv_num < 0) {
		UARTprintf((const char*) "recv failed (%d)\n", fdError());
		goto leave;
	...
	leave: switch (Recv_Array[AMJoinResultPKPos]) {
	case Join_Success:
...
int JB_JoinRequest(int Table_Index) {
	...
	// receive Server's Join Response
	recv_num = recv(server_sock, Recv_Array, JBJoinResponsePKLength,
			MSG_WAITALL);
	if (recv_num < 0) {
		UARTprintf((const char*) "recv failed (%d)\n", fdError());
		Join_Result = Join_Failed;
...
int PV_Periodic_Trans(int Head, int Tail) {
	...
	setsockopt(server_sock2, SOL_SOCKET, SO_SNDTIMEO, &server_timeout2,
			sizeof(server_timeout2));
	setsockopt(server_sock2, SOL_SOCKET, SO_RCVTIMEO, &server_timeout2,
			sizeof(server_timeout2));
	...
	do {
		...
		recv_num = recv(server_sock2, Recv_Array, PVPeriodicResponsePKLength,
				MSG_WAITALL);
		if (recv_num < 0) {
			UARTprintf((const char*) "recv failed (%d)\n", fdError());
			//goto leave;
	...
	if (try >= 5) {
		...
		UARTprintf(
				(const char*) "== Periodic Data Transmission Fail !!!! ==\n\n");
	}
	// Free Pointer & Socket
	if (Periodic_Array)
		free(Periodic_Array);
...
//// add extra parameter like AM fW version ....etc
	...
	Join_Array[AM_regi_Server_Cmd_NUM + i++] = (unsigned char) strlen(
			(const char*) AM_FWversion) + 1;
	memcpy(&Join_Array[AM_regi_Server_Cmd_NUM + i], AM_FWversion,
			strlen(AM_FWversion) + 1);
	i = i + strlen(AM_FWversion) + 1;

	Join_Array[AM_regi_Server_Cmd_NUM + i++] = AM_regi_Server_Cmd_Serip;
	sprintf(tempserver_ip, "%d.%d.%d.%d", G_ENVCONFIG.IP[0], G_ENVCONFIG.IP[1],
			G_ENVCONFIG.IP[2], G_ENVCONFIG.IP[3]);
	Join_Array[AM_regi_Server_Cmd_NUM + i++] = (unsigned char) strlen(
			(const char*) tempserver_ip) + 1;				///server ip length
	memcpy(&Join_Array[AM_regi_Server_Cmd_NUM + i], tempserver_ip,
			strlen(tempserver_ip) + 1);
...
int GenerateJBJoinCommandPK(unsigned char *Join_Array, int Table_Index) {
	...
	if (pv_info_table[Table_Index].Serial_Number[0] == 0xFF) {
		...
		for (i = 91; i < 115; i++)
			Join_Array[i] = pv_info_table[temp_Index].Device_Specification[i
					- 91];
		for (i = 115; i < 123; i++)
		...
		for (i = 91; i < 115; i++)
			Join_Array[i] = pv_info_table[Table_Index].Device_Specification[i
					- 91];
		for (i = 115; i < 123; i++)
			Join_Array[i] =
					pv_info_table[Table_Index].Manufacture_Date[i - 115];
	}
	return 0;
}

int GeneratePVPeriodicCommandPK(unsigned char *Periodic_Array, int Head,
		int Tail) {
	int i, j;
	int Index;
	int Count = 0;
	...
	i = 0;
	while (Index <= Tail) {
		if (member_table[Index].Valid
				>= 0&& member_table[Index].state ==JB_Join2Server) {
			if (pv_value_table[Index].Alert_State[0] == 0
					|| pv_value_table[Index].Alert_State[0] == 1) {

				for (j = 0; j < 6; j++)
					Periodic_Array[13 + i * 19 + j] =
							member_table[Index].MAC[j]; //MAC

				for (j = 1; j < 3; j++)
					Periodic_Array[13 + i * 19 + j + 6] =
							pv_value_table[Index].Voltage[j - 1]; //Payload
				for (j = 3; j < 5; j++)
					Periodic_Array[13 + i * 19 + j + 6] =
							pv_value_table[Index].Current[j - 3]; //Payload
				for (j = 5; j < 9; j++)
					Periodic_Array[13 + i * 19 + j + 6] =
							pv_value_table[Index].Power_Energy[j - 5]; //Payload
				for (j = 9; j < 13; j++)
					Periodic_Array[13 + i * 19 + j + 6] =
							pv_value_table[Index].Alert_State[j - 9]; //Payload

				Periodic_Array[13 + i * 19 + 6] =
						pv_value_table[Index].Diode_Temperature;	//Payload
			}
...
int TCP_Periodic_Link() {
	...
	// receive Server's Response
	recv_num = recv(server_sock, Recv_Array, TCPeriodicLINKResponsePKL,
			MSG_WAITALL);
		...
		for (j = 0; j < ResponseCPK; j++) {
			...
			case TcpLinkAMUpdatePeriod:
				...
				for (i = 0; i < 1; i++) {
					...
					TCP_Periodic_Link_time.Updata_Period[i] = Recv_Array[i + pos
							+ 1];
				}
			...
			case TcpLinkAMUpdateTCPP:
				...
				for (i = 0; i < 1; i++) {
					...
					TCP_Periodic_Link_time.TCP_Link_Period[i] = Recv_Array[i
							+ pos + 1];
				}
			...
			case TcpLinkAMResetJB:
				...
				if (!(JB_MAC = calloc(6, 1))) {
					UARTprintf(
							(const char*) "failed JB_MAC buffer allocation\n");
					break;
				}
				for (i = 0; i < 6; i++) {
			...
			case TcpLinkAMUpdateFW:
				...
				int remainder;
				Fsize = Recv_Array[pos + TCL_UpFwCmd_Offset_Len_LL]
						+ (Recv_Array[pos + TCL_UpFwCmd_Offset_Len_LH] << 8)
						+ (Recv_Array[pos + TCL_UpFwCmd_Offset_Len_HL] << 16)
						+ (Recv_Array[pos + TCL_UpFwCmd_Offset_Len_HH] << 24);

				UARTprintf((const char*) "Fsize : %d \n\n", Fsize);

				SegmentSizeCount = Fsize / Rev_SegmentSize;
				if (Fsize % Rev_SegmentSize)
					SegmentSizeCount++;

				for (i = 0; i < SegmentSizeCount; i++) {
					if (i == (SegmentSizeCount - 1))
						Rev_SegmentSize = Fsize % Rev_SegmentSize;

					recv_num = recv(server_sock, FW_Rev_SegmentArray,
							Rev_SegmentSize, MSG_WAITALL);
					UARTprintf(
					...
					if (recv_num) {
						...
						if (remainder)
							recv_num = recv_num - remainder;
						DynFlashProgram(FW_Rev_SegmentArray, BurnAddr,
								recv_num);
					}
				}
...
int SavePtime2Evn(char*Start) {
	...
	if (count == 4)
		Pollingtime = (*Start - '0') * 1000 + (*(Start + 1) - '0') * 100
				+ (*(Start + 2) - '0') * 10 + (*(Start + 3) - '0');
	if (count == 3)
		Pollingtime = (*Start - '0') * 100 + ((*(Start + 1) - '0')) * 10
				+ *(Start + 2) - '0';
	if (count == 2)
...
int SaveIP2Evn(char*Start) {
	...
	for (j = 0; j < 4; j++) {
		...
		if (Start == ptr + 1) {
			...
			G_ENVCONFIG.IP[i] = (*ptr - '0') * 100 + ((*(ptr + 1) - '0')) * 10
					+ *(ptr + 2) - '0';
			i++;
			ptr = Start + 1;
		} else
...
void Rewrite_Environment(void) {
	...
	sprintf(ServeripArray, "IP=%d.%d.%d.%d", G_ENVCONFIG.IP[0],
			G_ENVCONFIG.IP[1], G_ENVCONFIG.IP[2], G_ENVCONFIG.IP[3]);
	sprintf(AM_MacArray, "MAC=%x:%x:%x:%x:%x:%x", G_ENVCONFIG.Mac[0],
			G_ENVCONFIG.Mac[1], G_ENVCONFIG.Mac[2], G_ENVCONFIG.Mac[3],
			G_ENVCONFIG.Mac[4], G_ENVCONFIG.Mac[5]);
	sprintf(AM_PollingArray, "Polling_Time=%d", G_ENVCONFIG.Pollingtime[1]);
	sprintf(AM_SerierArray, "SN=%d%d%d%d%d%d%d%d", G_ENVCONFIG.SeriesNumber[0],
			G_ENVCONFIG.SeriesNumber[1], G_ENVCONFIG.SeriesNumber[2],
			G_ENVCONFIG.SeriesNumber[3], G_ENVCONFIG.SeriesNumber[4]\
,
			G_ENVCONFIG.SeriesNumber[5], G_ENVCONFIG.SeriesNumber[6],
			G_ENVCONFIG.SeriesNumber[7]);
...
#endif
	FlashErase(ptr);
	FlashProgram((uint32_t*) FW_Rev_SegmentArray, ptr,
			(EvnLen + 3) & (0xfffffffc));
	//UARTprintf((const char*)"\nErace Flash Finish\n");
}
